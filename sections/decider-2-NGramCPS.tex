...
\subsubsection{\ngramcps algorithm description}\label{sec:n-gramCPS:algo}
...
\begin{enumerate}
    ...
    \item \textbf{Right $n$-gram update.} In order to deal with the unknown symbol \texttt{?}, we look among the previously seen right $n$-grams (contained in set $R$ in Algorithm~\ref{alg:NGramCPS}) the ones that start by \texttt{00}, assume it is \texttt{000} and \texttt{001}. Then we add both local contexts \texttt{111 [C1] 000} and \texttt{111 [C1] 001}, if not already in, to our set of local configurations, see Algorithm~\ref{alg:NGramCPS}, l.\ref{alg:NGramCPS:line:insertInConfSet}.
\end{enumerate}

A symmetrical process is applied when the machine moves left (Algorithm~\ref{alg:NGramCPS}, l.\ref{alg:NGramCPS:line:moveLeft}).
